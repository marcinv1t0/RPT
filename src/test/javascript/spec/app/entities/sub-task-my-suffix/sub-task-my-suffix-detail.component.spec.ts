/* tslint:disable max-line-length */
import { ComponentFixture, TestBed, async } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { Observable } from 'rxjs/Observable';

import { RptTestModule } from '../../../test.module';
import { SubTaskMySuffixDetailComponent } from '../../../../../../main/webapp/app/entities/sub-task-my-suffix/sub-task-my-suffix-detail.component';
import { SubTaskMySuffixService } from '../../../../../../main/webapp/app/entities/sub-task-my-suffix/sub-task-my-suffix.service';
import { SubTaskMySuffix } from '../../../../../../main/webapp/app/entities/sub-task-my-suffix/sub-task-my-suffix.model';

describe('Component Tests', () => {

    describe('SubTaskMySuffix Management Detail Component', () => {
        let comp: SubTaskMySuffixDetailComponent;
        let fixture: ComponentFixture<SubTaskMySuffixDetailComponent>;
        let service: SubTaskMySuffixService;

        beforeEach(async(() => {
            TestBed.configureTestingModule({
                imports: [RptTestModule],
                declarations: [SubTaskMySuffixDetailComponent],
                providers: [
                    SubTaskMySuffixService
                ]
            })
            .overrideTemplate(SubTaskMySuffixDetailComponent, '')
            .compileComponents();
        }));

        beforeEach(() => {
            fixture = TestBed.createComponent(SubTaskMySuffixDetailComponent);
            comp = fixture.componentInstance;
            service = fixture.debugElement.injector.get(SubTaskMySuffixService);
        });

        describe('OnInit', () => {
            it('Should call load all on init', () => {
                // GIVEN

                spyOn(service, 'find').and.returnValue(Observable.of(new HttpResponse({
                    body: new SubTaskMySuffix(123)
                })));

                // WHEN
                comp.ngOnInit();

                // THEN
                expect(service.find).toHaveBeenCalledWith(123);
                expect(comp.subTask).toEqual(jasmine.objectContaining({id: 123}));
            });
        });
    });

});
